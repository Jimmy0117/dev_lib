...
block_fifo_t *g_p_fifo = NULL;
block_fifo_t *g_p_fifoAudio = NULL;
bool g_bPausedAudio = false;
bool g_bPausedVideo = false;
bool g_bFirstAudio = true;
bool g_bFirstVideo = true;
...
int GetLocalIp(vlc_object_t *p_parent, char *strIp)
{
	...
	if (ERROR_SUCCESS == nRel)
	{
		...
		while (pAdapter)
		{
			msg_Info(p_parent, "NetCard Count: %d", ++netCardNum);
			msg_Info(p_parent, "NetCard Name: %s", pAdapter->AdapterName);
			msg_Info(p_parent, "NetCard Describe: %s", pAdapter->Description);
			...
			do
			{
				msg_Info(p_parent, "IP Address[%d]", ++IPnumPerNetCard);
				msg_Info(p_parent, "IP Address: %s", pIpAddrString->IpAddress.String);
				// 排除掉VMware网卡
				if ( NULL == strstr(pAdapter->Description, "VMware Virtual Ethernet") )
				{
...
static void *UdpReceiveThread(void *p_data)
{
	...
	msg_Info(p_parent, "+++++++--->Enter UdpReceiveThread");
...
#else
	char hostname[256] = {0}, strIp[256] = {0};
	...
	if ( result == 0 )
	{
		msg_Dbg (p_parent, "gethostname sucess: %s\n", hostname);
	}
	else
	{
		msg_Err (p_parent, "gethostname failed");
	}
	/* client IP */
	...
	if ( hostent != NULL )
	{
		...
		{
			char *szIP = inet_ntoa( *( (struct in_addr *)hostent->h_addr_list[i] ) );
			if (szIP)
			{
				if (0 == strlen(strIp))
				{
				...
				{
					msg_Warn (p_parent, "++++++++playlist_Create, multi ip address");
					sprintf(strIp, "%s;%s", strIp, szIP);
				}	
			}
		}
	}
	...
	{
		msg_Err(p_parent, "gethostbyname failed");
	}
	msg_Info(p_parent, "++++++++get the local ip address: %s", strIp);
	...
	if( fd == -1 )
	{
		msg_Err( p_parent, "+++++++UdpReceiveThread, cannot open socket" );
		goto T_OUT;
	}
	for (;;)
	{
		...
		{
			msg_Err( p_parent, "++++++UdpReceiveThread, block_Alloc failed" );
			goto T_OUT;
		}
		...
		if( len < 0 )
		{
			...
			msg_Err( p_parent, "++++++UdpReceiveThread, net_Read failed" );
			goto T_OUT;
		}
		msg_Info(p_parent, "+++++++UdpReceiveThread,net_Read success (%d):0x%x 0x%x", 
							len, 
							p_block->p_buffer[0],
							p_block->p_buffer[1]);
...
T_OUT:
	...
	msg_Info(p_parent, "+++++++<---Leave UdpReceiveThread");
...
void TellClientNewVideo(decoder_t *p_dec)
{
	...
	if (( stream  = fopen("serverIp", "r" )) != NULL)
	{
		...
		msg_Dbg (p_dec, "$$$$$$$$$, TellClientNewVideo, fread serverIp return %d, %s", iRet, strIp);
		fclose(stream);	  
	}
	...
	{
		fclose(stream);
		msg_Dbg (p_dec, "++++++++TellClientNewVideo, remove playAfterPause");
...
static void DecoderProcess( decoder_t *p_dec, block_t *p_block )
{
	...
	static mtime_t smAudioPts = 0;
	static mtime_t smVideoPts = 0;
