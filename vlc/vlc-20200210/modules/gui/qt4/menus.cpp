                              ...
                              intf_thread_t *p_intf )
{
   ...
   // BAR_DADD( NavigMenu( p_intf, bar ), qtr( "P&layback" ), 3 );
    ...
    BAR_DADD( ViewMenu( p_intf, NULL, mi ), qtr( "查看" ), 4 );
...
// chenyj  remove menu item On View Menu
#if 0
    menu->addAction(
#ifndef __APPLE__
...
#else	 
	action = menu->addAction(QString::fromUtf8("全屏显示(学生)"), mi, SLOT(toggleAdvancedButtons()));
#endif
    action->setCheckable( true );
    ...
    /* Extensions 基于lua进行扩展 xujinxiang*/
