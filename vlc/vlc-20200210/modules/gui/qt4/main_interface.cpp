...
MainInterface::MainInterface( intf_thread_t *_p_intf ) : QVLCMW( _p_intf )
{
   ...
   // FirstRun::CheckAndRun( this, p_intf );
	...
	// chenyj set windows size to 800x600
#if 0

    QVLCTools::restoreWidgetPosition( settings, this, QSize(600, 420) );
#else
	 QSize defSize = QSize( 200, 400 );
...
void MainInterface::createMainWidget( QSettings *creationSettings )
{
	...
	controls = new ControlsWidget( p_intf, bFullScreenStudent, this );
#else
	...
	// chenyj remove right click menu
...
inline void MainInterface::showTab( QWidget *widget )
{
    ...
    if( b_autoresize )
	{
		msg_Dbg( p_intf, "##############showTab, resizeStack (%d %d)", stackWidgetsSizes[widget].width(), stackWidgetsSizes[widget].height() );
		// chenyj remove resize(-1, -1),fix shake
...
void MainInterface::releaseVideoSlot( void )
{
    ...
    // chenyj when release video , set bgwidget size
	bgWidget->resize(stackWidgetsSizes[stackCentralOldWidget].width(), stackWidgetsSizes[stackCentralOldWidget].height());

    /* We don't want to have a blank video to popup */
    stackCentralOldWidget = bgWidget;
	// chenyj add stackCentralOldWidget->show()
	msg_Dbg(p_intf, "XXXXXXXXXX, releaseVideoSlot,stackCentralOldWidget->show()");
	stackCentralOldWidget->show();
}
...
void MainInterface::videoSizeChanged( int w, int h )
{
	...
	{
		// chenyj modify w h .eg: (1066 600) -> (800, 450)
        //resizeStack( w, h );
	}
}
...
void MainInterface::setVLCWindowsTitle( const QString& aTitle )
{
    if( aTitle.isEmpty() )
    {
        ...
        setWindowTitle( qtr( "网络影院" ) );
    }
    else
    {
        //setWindowTitle( aTitle + " - " + qtr( "VLC media player" ) );
        //cjh 强制标题为网络影院，防止通过Find方式找不到
		setWindowTitle(qtr( "网络影院" ) );
    }
}
...
void MainInterface::updateSystrayTooltipName( const QString& name )
{
    ...
    else
    {
            ...
            ( i_notificationSetting == NOTIFICATION_MINIMIZED && (isMinimized() || isHidden()) ) )
        {
            ...
            sysTray->showMessage( qtr( "网络影院" ), name,
