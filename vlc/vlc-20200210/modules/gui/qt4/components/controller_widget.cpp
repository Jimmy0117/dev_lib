                           ...
                           b_is_muted( false ), b_ignore_valuechanged( false )
{
    ...
    // liyl
    {
        DWORD dwVolume = (DWORD)(volume * 100);
        if (ERROR_SUCCESS == ReadDwordFromRegedit(HKEY_LOCAL_MACHINE, REGPATH_VOLUME_PATH, REGPATH_VOLUME_KEY, dwVolume))
        {
            volume = (float)dwVolume / 100;
        }
    }
...
void SoundWidget::libUpdateVolume( float volume )
{
    ...
    if( i_volume != volumeSlider->value()  )
    {
        // liyl ¼ÇÂ¼ÉùÒô´óÐ¡
        DWORD dwVolume = (DWORD)(volume * 100);
        if (0 <= dwVolume && dwVolume <= 1000)
        {
            DWORD dwVolume = (DWORD)(volume * 100);
            WriteDwordToRegedit(HKEY_LOCAL_MACHINE, REGPATH_VOLUME_PATH, REGPATH_VOLUME_KEY, dwVolume);
        }        
