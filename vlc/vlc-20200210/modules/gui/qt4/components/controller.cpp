...
QWidget *AbstractController::createWidget( buttonType_e button, int options )
{
    ...
    case FRAME_BUTTON: {
        ...
        break;
		// chenyj remove fullscreen button
#if 0
    case FULLSCREEN_BUTTON:
    case DEFULLSCREEN_BUTTON:
        {
#if 1
		msg_Info( p_intf, "((((((((((( FULLSCREEN_BUTTON has Video %d", THEMIM->getIM()->hasVideo() );
#endif
    ...
    case QUIT_BUTTON: {
        ...
        break;
		// chenyj remove random and loop button
#if 0
    case RANDOM_BUTTON: {
                                ...
                                AbstractController( _p_i, _parent )
{
	...
	// chenyj convert "Advanced Controls" to "Full screen"
#if 0
...
void ControlsWidget::toggleAdvanced()
{
	...
	{
		if( (stream  = fopen( "fullscreenStudent", "a" )) != NULL )
		{
			fclose( stream ); 
		}
	}
	...
	if (( stream  = fopen("broadCastIp", "r" )) != NULL)
	{
		...
		msg_Dbg (p_intf, "toggleAdvanced, fread broadCastIp return %d, %s", iRet, strIpAddress);
		fclose(stream);	  
	}
	else
	{
		...
		char *cTempDou = NULL, *cTempMao = NULL;

		msg_Err (p_intf, "toggleAdvanced, fread broadCastIp failed");

		// CDCµØÖ·
#define		REG_CDC_ADDR_PATH			"Software\\Centerm\\TeacherClient"
#define		REG_CDC_ADDR				"CdcAddress"
#define		XCDC_SERVICE_ADDR			"http://239.1.1.1:80"
		...
		{
			msg_Dbg (p_intf, "CTRegReadString failed,Set Cdc Address to %s \n", XCDC_SERVICE_ADDR);
			strcpy(strAddrTemp, XCDC_SERVICE_ADDR);
		}
		if (FALSE == GetBroadIpFromDomain(strAddrTemp, strIpAddress))
		{
			sprintf(strIpAddress, "239.1.1.1");
			msg_Dbg (p_intf, "GetBroadIpFromDomain failed Set Cdc Address to %s \n", strIpAddress);
		}
	}
#endif
	...
	strMessage[0] = 0x80;
	strMessage[1] = 0x64;
	strMessage[2] = b_advancedVisible ? 0x0 : 0x1;
	msg_Dbg (p_intf, "tell client:player is fullscreenStudent, b_advancedVisible %d", b_advancedVisible);

	int fd = net_ConnectDgram( p_intf, strIpAddress, 5004, -1, 17);

	int iRet = send(fd, (const char *)strMessage, 3, 0 ); 
	msg_Dbg (p_intf, "tell client:player is fullscreenStudent, retsult(%d)", iRet);
#endif
