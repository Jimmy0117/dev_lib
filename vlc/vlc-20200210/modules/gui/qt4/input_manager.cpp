...
{
	// chenyj  disable short-jump-size
	return;
...
// liyl
bool IsTimePass(input_thread_t *p_input)
{
	...
	if (bTimeInit == false)
	{
		...
		tpend.tv_usec = 0;
		bTimeInit = true;
                msg_Dbg( p_input, "timeuse data init");
	}
	else
	{
		msg_Dbg( p_input, "timeuse has init.");
	}

	gettimeofday(&tpend,NULL); 
	msg_Dbg( p_input, "timeuse: end(%d.%d)t  start(%d.%d).", tpend.tv_sec, tpend.tv_usec, tpstart.tv_sec, tpstart.tv_usec);

	// 秒级 1.23456s
	double timeuse = (1000000 * (tpend.tv_sec - tpstart.tv_sec) + tpend.tv_usec-tpstart.tv_usec) / 1000000.0;
	msg_Dbg( p_input, "timeuse %f", timeuse);
	if (timeuse > 2.0 || timeuse < 0)
	{
	...
	{
		msg_Dbg( p_input, "timeuse no out, return false.");
		return false;
	}
}
